...
\(\mathrm{V6A}\), \(\mathrm{V6B}\), \(\mathrm{V6C}\), \(\mathrm{V8}\), and \(\mathrm{V9}\) admit direct Chan--Cooper binomial formulas
...
coefficients are defined.  The Chan--Cooper table gives the positive-$q$
coefficient sequence $U_8$; the atlas-normalized local \(\mathrm{V8}\)
branch uses $Z_8(-q)$, hence the coefficient sequence
$U_8^-(n)=(-1)^nU_8(n)$.  We record both because Lucas congruences are
preserved by this sign twist.

\begin{lemma}[Lucas summation closure]
\label{lem:lucas-summation-closure}
Let $S(n)=\sum_j f(n,j)$ be a finite integer-valued sum, with $f(n,j)=0$
outside its natural support.  Fix a prime $p$ and write
$n=n_0+pN$ and $j=j_0+pJ$, with $0\le n_0,j_0<p$.  Suppose the
carry-free terms satisfy
\[
...
\]
Consequently, if the hypothesis holds for every prime $p$, then $S$
satisfies Lucas congruences modulo every prime.
\end{lemma}
...
together with the Kummer carry criterion and Lemma~\ref{lem:lucas-summation-closure}.
...
BTY/Atkin--Lehner audit, and the \(\mathrm{V8}\) and \(\mathrm{V9}\) normalizer geometry remains useful
...
\end{center}
The June 2026 paper-facing addendum rows, including Hankel, Atkin--Lehner
$W_2$, and Chan--Cooper Lucas, are recorded separately in
\begin{center}
\texttt{data/relation\_atlas/}\\
\texttt{cf\_theorem\_transfer\_registry\_}\\
\texttt{2026\_06\_extension.json}
\end{center}
and summarized in the V2 registry/readout documents.  The excerpt is included
only to make the atlas semantics visible in the paper.
